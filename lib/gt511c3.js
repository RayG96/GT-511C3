 ...
 var GT511C3 = (function() {
   ...
   class Parser extends Transform {
     constructor() {
       ...
       this.mainBuff = Buffer.alloc(0);
     }
   
     _transform(chunk, encoding, cb) {
       // chunk is the incoming buffer here
       chunk = Buffer.concat([this.mainBuff, chunk]);
     if (chunk[0] == RESPONSE_START_CODE_1 && chunk[1] == RESPONSE_START_CODE_2 && chunk.length >= 12) {
       //lets take the 12bytes response
       var response = Buffer.from(chunk.slice(0, 12));
 
       //remaining bytes
       chunk = Buffer.from(chunk.slice(12));
     ...
     if (chunk[0] == DATA_START_CODE_1 && chunk[1] == DATA_START_CODE_2 && chunk.length >= DATA_PACKET_LENGTH) {
       //remove header
       var data = chunk.slice(0, DATA_PACKET_LENGTH);
 
       var crc = checkCRC(data);
 
       //remove 2 bytes CRC + header
       data = Buffer.from(data.slice(4, data.length - 2));
       
       //remaining bytes
       chunk = Buffer.from(chunk.slice(DATA_PACKET_LENGTH));
       ...
       this.mainBuff = Buffer.from(chunk);
       cb();
     }
   ...
   GT511C3.prototype.sendCommand = function(cmd, params) {
     ...
     return (new Promise(function(resolve, reject) {
       ...
       emitter.removeAllListeners('response_packet');
       emitter.on('response_packet', function(response) {
         if (DEBUG)
             ...
             if (err) {
               ...
               self.serialPort.drain(function(err) {
                 if (err) {
                   ...
                   if (DEBUG)
                     console.log(cmd.toString(16) + ' command sent!');
                 }
               });
             }
           });
     ...
     return (new Promise(function(resolve, reject) {
       ...
       function fun(baudrate, callback) {
         ...
         var open = function() {
           ...
           emitter.removeAllListeners('data_packet');
           emitter.on('data_packet', function(response) {
             if (DEBUG)
